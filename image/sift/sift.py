...
class SIFT:
    def __init__(self,img,params_dict):
        image = img.astype('float32')
        self.data={'img':image}
        self.params = params_dict
        # self.compute(params_dict)
    def compute(self,params_dict=None):
        if params_dict:
            self.params=params_dict
        img = self.data['img']
        blurred = self._blur(img)
        n_octave = int(round(np.log(min(blurred.shape)) / np.log(2) - 1))
        kernels=self._createGaussianKernels()
        gauss_img=self._applyGaussianKernels(blurred,n_octave,kernels)  # gauss_img: array of array(of images)
        dog=self._getDOG(gauss_img)
        keypoints = self._findScaleSpaceExtrema(gauss_img, dog)
        keypoints = SIFT._removeDuplicateKeypoints(keypoints)
        keypoints = SIFT._convertKeypointsToInputImageSize(keypoints)
        descriptors = self._generateDescriptors(keypoints, gauss_img)
        self.data['blurred']=blurred
        self.data['n_octave']=n_octave
        self.data['kernels']=kernels
        self.data['gauss_images']=gauss_img
        self.data['dog_images']=dog
    ...
    def _blur(self,image):
        ...
        sigma_diff = np.sqrt(max((params['sigma'] ** 2) - ((2 * params['assumed_blur']) ** 2), 0.01))
        blurred=cv2.GaussianBlur(image, (0, 0), sigmaX=sigma_diff, sigmaY=sigma_diff)
        return blurred

    def _createGaussianKernels(self):
        """
    ...
    def _applyGaussianKernels(self,image, n_octaves, gaussian_kernels):
        """
        Generate scale-space pyramid of Gaussian images
        """
        gaussian_images = []

        for octave_index in range(n_octaves):
    ...
    def plot_gauss_img(self):
        try:
            gauss_imgs=self.data['gauss_images']
        except:
            raise ValueError('Images Not Found.')
        self.plot_img_set(gauss_imgs)
    def plot_dog_img(self):
        try:
            dog_imgs=self.data['dog_images']
        except:
            raise ValueError('Images Not Found.')
        self.plot_img_set(dog_imgs)

    def _getDOG(self,gaussian_images):
        """
    ...
    def _findScaleSpaceExtrema(self,gaussian_images, dog_images):
        """Find pixel positions of all scale-space extrema in the image pyramid
        """
        params = self.params
        num_intervals = params['num_intervals']
        image_border_width = params['image_border_width']
            ...
            for image_index, (first_image, second_image, third_image) in enumerate(
                ...
                for i in range(image_border_width, first_image.shape[0] - image_border_width):
                    for j in range(image_border_width, first_image.shape[1] - image_border_width):
                        if SIFT._isPixelAnExtremum(first_image[i - 1:i + 2, j - 1:j + 2],
                                             second_image[i - 1:i + 2, j - 1:j + 2],
                                             third_image[i - 1:i + 2, j - 1:j + 2], threshold):
                            localization_result = self._localizeExtremumViaQuadraticFit(i, j, image_index + 1, octave_index,dog_images_in_octave)
                            if localization_result is not None:
                                keypoint, localized_image_index = localization_result
                                keypoints_with_orientations = self._computeKeypointsWithOrientations(keypoint, octave_index,gaussian_images[octave_index][localized_image_index])
        ...
        if abs(center_pixel_value) > threshold:
            if center_pixel_value > 0:
                ...
                return np.all(center_pixel_value <= first_subimage) and \
                       ...
                       center_pixel_value <= second_subimage[1, 2]
        return False
    def _localizeExtremumViaQuadraticFit(self,i, j, image_index, octave_index, dog_images_in_octave, eigenvalue_ratio=10,num_attempts_until_convergence=5):
        ...
        for attempt_index in range(num_attempts_until_convergence):
            ...
            gradient = SIFT._computeGradientAtCenterPixel(pixel_cube)
            hessian = SIFT._computeHessianAtCenterPixel(pixel_cube)
    ...
    def _computeKeypointsWithOrientations(self,keypoint, octave_index, gaussian_image, radius_factor=3, num_bins=36,
                                         peak_ratio=0.8, scale_factor=1.5, float_tolerance=1e-7):
    ...
    def _removeDuplicateKeypoints(keypoints):
        ...
        keypoints.sort(key=cmp_to_key(SIFT._compareKeypoints))
    ...
    def _generateDescriptors(self,keypoints, gaussian_images, window_width=4, num_bins=8, scale_multiplier=3,
        ...
        for keypoint in keypoints:
            octave, layer, scale = SIFT._unpackOctave(keypoint)
            ...
            half_width = int(round(hist_width * np.sqrt(2) * (
                        window_width + 1) * 0.5))  # sqrt(2) corresponds to diagonal length of a pixel
            half_width = int(
                min(half_width, np.sqrt(num_rows ** 2 + num_cols ** 2)))  # ensure half_width lies within image
